...
class Slurm(ExecutionProvider):
    ...
    def _status(self):
        ...
        retcode, stdout, stderr = execute_wait(
            "squeue --job {0}".format(job_id_list), 3)
        ...
        # blanks for missing jobs, we might lose some information about why the
        # jobs failed.
    ...
    def submit(self, cmd_string, blocksize, job_name="parsl.auto"):
        ...
        if self.current_blocksize >= self.config["execution"]["options"]["max_parallelism"]:
            logger.warn(
                "[%s] at capacity, cannot add more blocks now",
                self.sitename)
            return None
    ...
    def cancel(self, job_ids):
        ...
        retcode, stdout, stderr = execute_wait(
            "scancel {0}".format(job_id_list), 3)
        rets = None
        if retcode == 0:
            for jid in job_ids:
                # Setting state to cancelled
                self.resources[jid]['status'] = translate_table['CA']
    ...
    def scale_in(self, size):
        ...
        if not self.resources:
            ...
            for resource in self.resources[0:size]:
                ...
                retcode, stdout, stderr = execute_wait(
                    "scancel {0}".format(resource['job_id']), 3)
