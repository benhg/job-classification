...
def f3(x, y, vx, vy, t, GAMMA):
    return -(GMs * x) / ((x**2 + y**2)**(3 / 2 + CORR)) * \
        (1 + (GAMMA) / (x**2 + y**2))


def f4(x, y, vx, vy, t, GAMMA):
    return -(GMs * y) / ((x**2 + y**2)**(3 / 2 + CORR)) * \
        (1 + (GAMMA) / (x**2 + y**2))
...
def solar_system_rk4(r0, v0, f1, f2, f3, f4, tmax, gamma):
    ...
    for t in tpoints:
        ...
        l2 = h * f2(x + k1 / 2, y + l1 / 2, vx +
                    m1 / 2, vy + n1 / 2, t + h / 2)
        m2 = h * f3(x + k1 / 2, y + l1 / 2, vx + m1 /
                    2, vy + n1 / 2, t + h / 2, gamma)
        n2 = h * f4(x + k1 / 2, y + l1 / 2, vx + m1 /
                    2, vy + n1 / 2, t + h / 2, gamma)
        k3 = h * f1(x + k2 / 2, y + l2 / 2, vx +
                    m2 / 2, vy + n2 / 2, t + h / 2)
        l3 = h * f2(x + k2 / 2, y + l2 / 2, vx +
                    m2 / 2, vy + n2 / 2, t + h / 2)
        m3 = h * f3(x + k2 / 2, y + l2 / 2, vx + m2 /
                    2, vy + n2 / 2, t + h / 2, gamma)
        n3 = h * f4(x + k2 / 2, y + l2 / 2, vx + m2 /
                    2, vy + n2 / 2, t + h / 2, gamma)
