...
def numerator_of_integrated_function(x, y, z):
    ...
    def f(theta):
        return cross([-3 * sin(theta), 3 * cos(theta), 0],
                     [x - 3 * cos(theta), y - 3 * sin(theta), z])
...
def denominator_of_integrand(x, y, z):
    ...
    def g(theta):
        return ((x - 3 * cos(theta))**2 +
                (y - 2 * sin(theta))**2 + (z)**2)**(3 / 2)
...
def trapezoid_integral(f, a, b, h):
    return h * (f(a) / 2 + f(b) / 2 +
                sum([f(a + (j * h)) for j in range(1, int((b - a) / h))]))
...
def adaptive_trapezoidal(f, a, b, delta):
    ...
    while err.all() > delta:
        ...
        n *= 2
        next_int = I_1 / 2 + h * \
            (sum([f(a + k * h) for k in range(1, int(n), 2)]))
