...
class rudpClient:
        ...
        for i in range(MAX_RESND):
            try:
                ...
                while True:
                    ...
                    try:
                        recvPkt = decode(recvData)
                        sendPkt = rudpProcessSwitch[recvPkt['pktType']](
                            recvPkt, self.conn)
                        return True
                    except WRONG_PKT as KeyError:
                        continue
            except timeout:
                continue
        ...
        for i in range(0, len(data_pkt), 1):
            ...
            for j in range(MAX_RESND):
                try:
                    ...
                    print(
                        "send:  'pktType':{}, 'pktId':{}, 'data':{} bytes".format(
                            sendPkt['pktType'], sendPkt['pktId'], len(
                                sendPkt['data'])))  # For debugging
                    while True:
                        recvData, addr = self.skt.recvfrom(MAX_DATA)
                        try:
                            recvPkt = decode(recvData)
        ...
        for i in range(MAX_RESND):
            try:
                ...
                while True:
                    ...
                    try:
                        recvPkt = decode(recvData)
                        rudpProcessSwitch[recvPkt['pktType']](
                            recvPkt, self.conn)
                    except WRONG_PKT as KeyError:
                        continue
    ...
    def sendFile(self, filepath):
        ...
        # Block size is the size of data that can be sent by calling sendData()
        # every time
