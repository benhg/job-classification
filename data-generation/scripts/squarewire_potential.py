...
def adaptive_trapezoidal_2d(f, ax, ay, bx, by, delta):
    ...
    while err > delta:
        ...
        nx *= 2
        ny *= 2
        next_int = I_1 / 2 + h * (sum([f(ax + k * h,
                                         ay) for k in range(1,
                                                            int(nx),
                                                            2)])) + h * (sum([f(ax,
                                                                                ay + k * h) for k in range(1,
                                                                                                           int(
                                                                                                               ny),
                                                                                                           2)]))
...
print(("The total field at point(x, y) = (0, 0) is {}".format(
    adaptive_trapezoidal_2d(function_to_integrate_00, -
                            1 / 2, -1 / 2, -1 / 2, 1 / 2, 10**-3)[0]
    + adaptive_trapezoidal_2d(function_to_integrate_00, -
                              1 / 2, -1 / 2, 1 / 2, -1 / 2, 10**-3)[0]
    + adaptive_trapezoidal_2d(function_to_integrate_00,
                              1 / 2, -1 / 2, 1 / 2, 1 / 2, 10**-3)[0]
...
print(("The total field at point(x, y) = (1/4, 0) is {}".format(
    adaptive_trapezoidal_2d(function_to_integrate_140, -
                            1 / 2, -1 / 2, -1 / 2, 1 / 2, 10**-3)[0]
    + adaptive_trapezoidal_2d(function_to_integrate_140, -
                              1 / 2, -1 / 2, 1 / 2, -1 / 2, 10**-3)[0]
    + adaptive_trapezoidal_2d(function_to_integrate_140,
...
for i, loc1 in enumerate(linspace(-SIZE, SIZE, 100)):
    def worker(tup):
        ...
        def total_integral(x, y):
            inte = adaptive_trapezoidal_2d(
                function_to_integrate_140, -1 / 2, -1 / 2, -1 / 2, 1 / 2, 10**-3)[0] + adaptive_trapezoidal_2d(
                function_to_integrate_140, -1 / 2, -1 / 2, 1 / 2, -1 / 2, 10**-3)[0] + adaptive_trapezoidal_2d(
                function_to_integrate_140, 1 / 2, -1 / 2, 1 / 2, 1 / 2, 10**-3)[0] + adaptive_trapezoidal_2d(
                function_to_integrate_140, -1 / 2, 1 / 2, 1 / 2, 1 / 2, 10**-3)[0]
            if y == 0:
...
# AFTER THE PHI EXISTS, THIS PART SHOULD JUST WORK. TRANSLATE X,Y TO
# POINTS IN PHI
